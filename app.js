...
(() => {
  ...
  const TEMPLATE_LIBRARY = [
    ...
    },
    {
      ...
      items: [
        ...
        { name: "Food / meal log",  icon: "📓", color: "#6366f1", category: "Nutrition", time: "All day", notes: "Jot what you eat — awareness alone often trims 10-20% without willpower." },
        { name: "Veggies first",    icon: "🥦", color: "#22c55e", category: "Nutrition", time: "All day", notes: "Fill half your plate with vegetables before the rest — fibre and volume keep you full." },
        { name: "Eat slowly (20 min)", icon: "🍽️", color: "#14b8a6", category: "Nutrition", time: "All day", notes: "Aim for ~20 minutes per meal so fullness signals catch up before you overeat." },
        { name: "No added sugar",   icon: "🍬", color: "#ef4444", category: "Nutrition", time: "All day", notes: "Skip added sugar today — a high-impact, keepable win. Your clean streak keeps count.", quit: true },
        { name: "Strength 2×/week", icon: "💪", color: "#f59e0b", category: "Fitness", time: "Morning", notes: "Lifting preserves muscle in a deficit, so you lose fat rather than muscle.", freqType: "weekly", weeklyTarget: 2 },
        { name: "Sleep 7+ hours",   icon: "😴", color: "#a855f7", category: "Sleep", time: "10:30 PM", notes: "Short sleep drives hunger and cravings — protecting 7-8 hours makes the deficit far easier.", nightPrevDay: true },
        { name: "Hunger check before snacking", icon: "✋", color: "#3b82f6", category: "Custom", time: "All day", notes: "Pause and ask: actually hungry, or just bored, thirsty, or tired?" },
        { name: "Fibre — 30 g",     icon: "🌾", color: "#22c55e", category: "Nutrition", time: "All day", notes: "Fibre fills you up and steadies blood sugar. Build up gradually with water.", type: "count", target: 30, unit: "g", increment: 5 },
      ],
    },
    {
  ...
  function recoveryScore(s) {
    if (!s) return null;
    const clamp = (n) => Math.max(0, Math.min(100, n));
    const parts = [];
    if (s.sleepHrs != null)  parts.push(clamp(((s.sleepHrs - 4) / 4) * 100));        // 4h→0, 8h→100
    if (s.hrvMs != null)     parts.push(clamp(((s.hrvMs - 20) / 60) * 100));         // 20→0, 80→100
    if (s.restingHr != null) parts.push(clamp(((80 - s.restingHr) / 40) * 100));     // 80→0, 40→100
    if (s.spo2 != null)      parts.push(clamp(((s.spo2 - 90) / 8) * 100));           // 90→0, 98→100
    if (s.respRate != null)  parts.push(clamp(100 - Math.abs(s.respRate - 14) * 8)); // ideal ~14/min
    if (s.skinTempC != null) parts.push(clamp(100 - Math.abs(s.skinTempC) * 40));    // near baseline → high
    if (!parts.length) return null;
    const score = Math.round(parts.reduce((a, b) => a + b, 0) / parts.length);
    const tier = score >= 75 ? "good" : score >= 50 ? "mid" : "low";
    const label = tier === "good" ? "Ready to train" : tier === "mid" ? "Moderate — take it steady" : "Prioritise rest";
  ...
  function latestRecovery() {
    ...
    return keys.length ? Object.assign({ dateKey: keys[keys.length - 1] }, r.days[keys[keys.length - 1]]) : null;
  }
  async function syncRecovery() {
    ...
    try {
      ...
      const snap = { hrvMs, restingHr, spo2, respRate,
        sleepHrs: sleepHrs > 0 ? sleepHrs : null, skinTempC, updatedAt: Date.now() };
      const any = ["hrvMs", "restingHr", "spo2", "respRate", "sleepHrs", "skinTempC"].some((k) => snap[k] != null);
      ...
      const r = loadRecovery();
      r.days[dateKey(new Date())] = snap;
      r.updatedAt = Date.now();
      saveRecovery(r);
      renderRecovery();
      showToast("🫀 Recovery synced", "success");
    } catch (e) {
  ...
  // Focus mode (Today): each day-part starts collapsed and a tapped section
  // auto-closes after a short delay. `expandedParts` holds the open ones.
